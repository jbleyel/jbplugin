...
/// events by adding a delegate to <see cref="RegistryChanged"/>.
...
///         monitor.RegistryChanged += new EventHandler(RegistryChanged);
///         monitor.Start();
///
///         while(true);
///     }
...
public class RegistryMonitor : IDisposable
{
    ...
    private IntPtr GetRegistryHandle(RegistryKey registryKey)
    {
        // hiveKey = Registry.LocalMachine.OpenSubKey(subKey);
        Type type = typeof(RegistryKey);
        FieldInfo fi = type.GetField("hkey", BindingFlags.NonPublic | BindingFlags.Instance);
        SafeHandle handle = (SafeHandle)fi.GetValue(registryKey);
        ...
        //return ((System.Runtime.InteropServices.SafeHandle)(registryKey.hkey)).handle;
    }
            ...
            while (!eventTerminate.WaitOne(0, true))
            {
                if (WaitForChange())
                {
                    if (RegistryChanged != null)
                        RegistryChanged(this, EventArgs.Empty);
                }
            }
        }
        catch (Exception e)
